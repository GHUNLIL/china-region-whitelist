...
CN_OPERATOR_ACCESS_FILE="${CN_OPERATOR_ACCESS_FILE:-${DATA_DIR}/country/CN-ISP.txt}"
...
cn_set_data_dir() {
  ...
  CN_OPERATOR_ACCESS_FILE="${DATA_DIR}/country/CN-ISP.txt"
...
cn_use_runtime_data_if_available() {
  if [[ -s "${CN_RUNTIME_DIR}/data/regions.json" && -d "${CN_RUNTIME_DIR}/data/regions" && -s "${CN_RUNTIME_DIR}/data/country/CN.txt" && -s "${CN_RUNTIME_DIR}/data/country/CN-ISP.txt" ]]; then
...
cn_validate_prebuilt_data_dir() {
  local data_dir="$1"
  if [[ ! -s "${data_dir}/regions.json" || ! -s "${data_dir}/regions.tsv" || ! -s "${data_dir}/country/CN.txt" || ! -s "${data_dir}/country/CN-ISP.txt" || ! -d "${data_dir}/regions" ]]; then
...
cn_collect_cidrs() {
  ...
  for code in "$@"; do
    if cn_is_operator_access_selector "${code}"; then
      cn_collect_operator_access_cidrs
      continue
    fi
...
cn_collect_operator_access_cidrs() {
  if [[ ! -r "${CN_OPERATOR_ACCESS_FILE}" ]]; then
    echo "缺少三大运营商和教育网 CIDR 文件：${CN_OPERATOR_ACCESS_FILE}" >&2
    echo "请重新运行 bootstrap.sh 或 install.sh update-data 拉取 GitHub 预制数据。" >&2
    return 1
  fi
  sed 's/[[:space:]]*$//' "${CN_OPERATOR_ACCESS_FILE}" | awk 'NF && $0 !~ /^#/'
}
...
cn_is_operator_access_selector() {
  case "$1" in
    CN-ISP|cn-isp|ISP|isp|三网|三网教育|运营商|三大运营商|三大运营商和教育网|三大运营商与教育网)
      return 0
      ;;
  esac
  return 1
...
cn_collect_selector_cidrs() {
  ...
  while IFS= read -r selector; do
    ...
    [[ -n "${selector}" ]] || continue
    if cn_is_operator_access_selector "${selector}"; then
      cn_collect_operator_access_cidrs
    elif cn_is_all_china_selector "${selector}"; then
      cn_collect_country_cidrs
...
cn_validate_port_policies() {
  ...
  for raw_policy in "${policy_items[@]}"; do
    ...
    while IFS= read -r selector; do
      selector="$(cn_trim "${selector}")"
      [[ -n "${selector}" ]] || continue
      if cn_is_operator_access_selector "${selector}" || cn_is_all_china_selector "${selector}"; then
        true
      elif [[ "${selector}" =~ ^[Aa][Ss][0-9]+$ ]]; then
        asn="$(cn_normalize_asn "${selector}")" || return 1
        [[ -n "${asn}" ]] || return 1
...
cn_save_config() {
  ...
  {
    echo "# Generated by china-region-whitelist. CN_CODES may contain CN, CN-ISP, or province codes."
    printf 'CN_CODES="%s"\n' "${codes[*]}"
    printf 'CN_ASNS="%s"\n' "${asns}"
