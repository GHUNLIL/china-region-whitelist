...
脚本默认使用键盘菜单交互：上/下键移动，空格勾选或取消，回车确认。省份菜单里可以直接勾选 `全国（中国大陆全部省级 IP）`，也可以按省/自治区/直辖市逐个勾选。

省份选择后，脚本会用菜单询问是否追加 ASN 白名单。这个功能适合把国外管理服务器所在云厂商 ASN 加进去，避免省份白名单生效后海外管理机无法登录。选择输入 ASN 后，可以填 `AS16509 AS14061` 这种格式，也可以留空。

然后脚本会进入端口优先白名单菜单。你可以选择“添加一条端口策略”，输入单端口或端口范围，再用同样的上下键/空格菜单勾选这个端口允许的省份；如果还需要国外管理 ASN 或固定 IP，再补充输入 ASN/IP/CIDR。

端口策略优先级高于整机默认白名单：如果某个端口命中了端口策略，来源必须匹配该端口自己的白名单，否则即使来源在整机默认白名单里也会被拒绝。
