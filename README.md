...
仓库会通过 GitHub Actions 每小时同步一次上游 CIDR 数据。服务器运行 `apply` 或 `dry-run` 时，默认还会在本机再同步一次上游数据到 `/var/lib/po0-region-whitelist`，确保配置时使用的是当时可获取到的最新数据。
...
```bash
git clone https://github.com/GHUNLIL/po0-whitelist.git
cd po0-whitelist
```
...
`apply` 成功后会保存选择到 `/etc/po0-region-whitelist.conf`，并安装 `po0-region-whitelist.service`。服务器重启后，systemd 会自动按保存的地区配置恢复 ipset/iptables 规则；恢复时会尝试更新数据，更新失败则使用已有运行时数据或仓库内置数据。
...
```bash
sudo PO0_DATA_BASE_URL=https://your-mirror.example/iplist/data/cncity bash install.sh apply
```
